...
print(tabulate(t2_df.head(), headers="keys", tablefmt="fancy_grid"))
...
print(tabulate(t2_const_df.head(), headers="keys", tablefmt="fancy_grid"))
...
forward_df = gaussian_stack_forward(
    ...
    config["PLUME_HEIGHT"], 2500, naive_phi_steps, (
...
axs[0].plot(t2_const_df["radius"].values,
            t2_const_df["MassArea"].values, 'C2o', label="Tephra2 (Const. Wind)")
...
#         display(data[["Contributions", "MassArea"]])
    ...
    for j, phi_step in enumerate(phi_new):
        ...
        vis.plot_residuals(post_df, vent=(0, 0), values="mass_res", plot_type="size",
                           ...
                           ax=axs[j], wind=(params_list[i]["u"]*20, params_list[i]["v"]*20))
        axs[j].set_xlim([-3000, 6500])
        axs[j].set_ylim([-500, 12000])
    plt.tight_layout()
    plt.show()
