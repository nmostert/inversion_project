...
def diffusion_transform(D_star, top, bottom):
    D = (top-bottom)*np.exp(-np.exp(-D_star))+bottom
    return D

def diffusion_inv_transform(D, bottom, top):
    D_star = -np.log(-np.log((D-bottom)/(top-bottom)))
    return D_star
...
def total_misfit(k, setup, z, diff_factors, TGSD=None, total_mass=None, transformed=True, debug=False):
    ...
    if transformed:
        ...
        u = k[3]
        v = k[4]
        diffusion_coefficient = diffusion_transform(k[5], param_transform(k[6])*diff_factors[0], param_transform(k[6])*diff_factors[1])
...
def custom_minimize(func, old_k, old_TGSD, old_total_mass, setup, z, H, include_idxes, exclude_idxes, all_params, diff_factors, sol_iter=10, max_iter=200, tol=0.1, adjustment_factor=0.5, adjust_mass=True, adjust_TGSD=True):
    ...
    old_misfit, contributions_old, pred_masses_old = total_misfit(all_params, setup, z, diff_factors,
    ...
    k_star += [u, v,
               diffusion_inv_transform(D, ftt*diff_factors[0], ftt*diff_factors[1]),
    ...
    new_k += [u_star, v_star,
               diffusion_transform(D_star, param_transform(ftt_star)*diff_factors[0], param_transform(ftt_star)*diff_factors[1]),
    ...
    new_misfit, contributions_new, pred_masses_new = total_misfit(new_k, setup, z, diff_factors, 
    ...
    if sol.success == False and sol.status is not 2:
        ...
        if (conv_crit < tol) or (new_misfit < 1e-5):
            ...
            return custom_minimize(func, np.array(new_k, dtype=np.float64)[include_idxes], new_TGSD, new_total_mass, setup, z, H, include_idxes, exclude_idxes, all_params, diff_factors, sol_iter, max_iter, tol, adjustment_factor, adjust_mass, adjust_TGSD)
...
def gaussian_stack_inversion(
    ...
    trans_vals += [guesses["u"],
                   guesses["v"],
                   diffusion_inv_transform(guesses["D"], guesses["ftt"]*diff_factors[0], guesses["ftt"]*diff_factors[1]),
    ...
    def func(k):
        ...
        mf, _, _ = total_misfit(kt, setup, z, diff_factors)
        return mf
    ...
    ret = custom_minimize(func, k0, TGSD_TRACE[0], MASS_TRACE[0], 
        setup, z, column_cap, include_idxes, exclude_idxes, all_params, diff_factors,
    ...
    misfit, contributions, pred_masses = total_misfit(sol_vals, setup, z, diff_factors, TGSD=new_tgsd, total_mass=new_total_mass, transformed=False)
...
def gaussian_stack_multi_run(
    ...
    out="verb", column_cap=45000, diff_factors=(0.5, 1.4), runs=5, pre_samples=1,
    ...
    while i < runs:
        ...
        pre_priors_list, pre_misfit_list = generate_hypercube_samples(
            ...
            z_min, z_max, elevation, phi_steps, 
            total_mass, eddy_constant, column_cap, diff_factors
        )
        ...
        output = gaussian_stack_inversion(
            ...
            column_cap=column_cap, diff_factors=diff_factors, out=out,
...
def gaussian_stack_genetic_optimisation(
    ...
    z_min, z_max, elevation, 
    phi_steps, total_mass, param_config, eddy_constant=.04, 
    out="verb", column_cap=45000, generations=5, diff_factors=(0.5, 1.4),
    sol_iter=20, max_iter=200, tol=0.01,
    adjust_TGSD=True, adjust_mass=True, 
    adjustment_factor=0.5, 
    ...
    for p in range(population_size):
        ...
        misfit, _, _ = get_error_contributions(
            data, num_points, column_steps, 
            z_min, z_max, elevation, phi_steps, diff_factors, 
...
def generate_hypercube_samples(
    ...
    for p in range(sample_size):
        ...
        misfit, _, _ = get_error_contributions(
            data, num_points, column_steps, 
            z_min, z_max, elevation, phi_steps, 
            samples, total_mass, diff_factors, eddy_constant=eddy_constant, column_cap=column_cap)
...
def generate_param_samples(
    sample_size, param_config, data, num_points, column_steps, 
    z_min, z_max, elevation, phi_steps, 
    total_mass, eddy_constant, diff_factors, column_cap
):
    sample_misfit_list = []
    sample_list = []
    for p in range(sample_size):
...
def get_error_contributions(    
    data, num_points, column_steps, 
    z_min, z_max, elevation, 
    phi_steps, params, total_mass, diff_factors, eddy_constant=.04, column_cap=45000, debug=False
):
    global AIR_VISCOSITY, GRAVITY, AIR_DENSITY
    ...
    misfit, contributions, _ = total_misfit(param_vals, setup, z, diff_factors, total_mass=total_mass, TGSD=TGSD, transformed=False, debug=debug)
