...
def custom_minimize(func, old_k, old_TGSD, old_total_mass, setup, z, H, include_idxes, exclude_idxes, all_params, gof="chi-sqr", sol_iter=10, max_iter=200, tol=0.1, adjustment_factor=0.5, adjust_mass=True, adjust_TGSD=True):
    ...
    for j in range(len(setup)):
        ...
        new_mass_in_phi += [(1-adjustment_factor)*mass_in_phi +
                            adjustment_factor*mass_in_phi*adjustment]

        print("%s \t%.2e \t%.2e \t%.2e \t%.4f \t\t%.4f" % (str(j), mass_in_phi, sum(
            pred_masses_old[j]), sum(obs_masses[j]), adjustment, adjustment_factor))
    ...
    sol = minimize(func, np.array(k_star, dtype=np.float64)[
                   include_idxes], method="Nelder-Mead", options={'maxiter': sol_iter, 'disp': True})
    ...
    if sol.success is False and sol.status != 2:
        ...
        # we're using our own convergence criteria.
...
def gaussian_stack_inversion(
    ...
    misfit, contributions, pred_masses = total_misfit(
        sol_vals, setup, z, gof=gof, TGSD=new_tgsd, total_mass=new_total_mass, transformed=False)
...
def gaussian_stack_multi_run(
    ...
    while i < runs:
        ...
        if status is False:
            ...
            display(pd.DataFrame([pre_priors_list[best_prior], params], index=[
                    "Priors", "Posteriors"]).T)
...
def gaussian_stack_genetic_optimisation(
    ...
    while i < generations:
        ...
        offspring_list, tgsd_offspring, mass_offspring = weighted_selection_crossover(
            mating_pool, misfit_list, mating_pool_idx, param_config, num_offspring, adjust_TGSD, adjust_mass)
        misfit_list = []
        tgsd_list = []
        mass_list = []
        column_list = []
        ...
        for offspring, in offspring_list:
            ...
            display(pd.DataFrame([offspring, params],
                    index=["Priors", "Posteriors"]).T)
...
def annealed_gaussian_stack_genetic_optimisation(
    ...
    while i < generations:
        ...
        offspring_list, tgsd_offspring, mass_offspring = weighted_selection_crossover(
            mating_pool, misfit_list, mating_pool_idx, param_config, num_offspring, adjust_TGSD, adjust_mass, migrants=migrant_list)
        misfit_list = []
        tgsd_list = []
        mass_list = []
        column_list = []
...
def get_error_contributions(
    ...
    misfit, contributions, _ = total_misfit(
        param_vals, setup, z, gof=gof, total_mass=total_mass, TGSD=TGSD, transformed=False, debug=debug)
